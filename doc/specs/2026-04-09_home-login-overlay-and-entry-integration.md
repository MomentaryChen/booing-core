...
Status: **進行中** — **工程交付已合入**；**產品結案**待 §12（QA §11 證據、規格定稿、最終 reviewer、選填 UX 簽收）完成後歸檔至 `doc/specs/done/`。

## Related specs

- 統一登入與 RBAC 已結案主檔：`doc/specs/done/2026-04-08_unified-login-rbac-delivery-plan.md`
- 首頁 intro 視覺已結案：`doc/specs/done/2026-04-09_home-intro-page-redesign.md`
- 視覺資源準則：`doc/specs/done/2026-04-09_visual-assets-dashboard-guidelines.md`
...
- [ ] **§3／§5／§10** 與目前 `main`（或發佈分支）行為一致，無互相矛盾。
- [ ] **`qa-agent`**：§11 全列執行完畢，附證據（建議路徑 `qa-agent/artifacts/screenshots/` 或團隊約定）；更新 README 截圖索引時 **中英 README 同步**。
- [ ] **`pnpm run build`**（`frontend/`）或 CI 通過之 **commit／日期** 已寫入本節或 PR。
- [ ] **`reviewer-agent`**：結案前最後一次程式／規格變更後一輪審查；**無未解 high/critical**（僅移檔 `done/` 且零 diff 者依團隊慣例可註記豁免）。
- [ ] （選填）**uiux-agent** sign-off 連結或「已知視覺債」註記。
- [ ] PM 將 **Status** 改為 **已結案**，並依 `spec-done-archive.mdc` **僅保留一份**移至 `doc/specs/done/2026-04-09_home-login-overlay-and-entry-integration.md`，**grep** 更新引用：`2026-04-09_home-login-overlay-and-entry-integration`、`doc/specs/2026-04-09_home-login-overlay`（範圍：`.agents/skills/`、`README.md`、`README.zh-TW.md`、`doc/`、`.cursor/`、程式註解）。

**歸檔後：** 本檔開頭註明「已移至 `doc/specs/done/`」；連到**活躍**規格用 `../`，連到同批 `done/` 用 `./`。
